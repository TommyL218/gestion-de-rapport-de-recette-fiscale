...
const ConsolidationAnnuelA = () => {
  ...
  const fetchData = () => {
    if (selectedYear && token) {
      axios.post(`http://localhost:5000/recette/getConsolidationAnnuelleA`, {
        annee: selectedYear,
      }, {
        headers: {
...
const exportToExcel = () => {
  ...
  const worksheetData = [
   [`Consolidation DRI ${selectedYear}`],
  ...
  XLSX.writeFile(workbook, `Consolidation_${selectedYear}_${nom_departement}.xlsx`);
};
  // Export PDF
  const exportToPDF = () => {
    const doc = new jsPDF();
...
export default ConsolidationAnnuelA;
