...
const PrevisionByTrimestreA = () => {
  ...
  useEffect(() => {
    if (selectedYear && selectedTrimestre && token) {
      axios.get(`http://localhost:5000/prevision/getByTrimestreD/${selectedYear}/${selectedTrimestre}/${selectedCentre}`, {
        headers: {
  ...
  }, [selectedYear, selectedTrimestre, selectedCentre, token]);
    // Fetch departement
    useEffect(() => {
  ...
  const exportToExcel = () => {
    ...
    const wsData = [
      ...
      ['TYPE D\'IMPÔT', ...mois.map(mois => mois.toUpperCase()), `TOTAL TRIMESTRE(${selectedTrimestre})`, 'TRIMESTRE', 'ANNEE'], // En-têtes avec "ANNEE"
      ...previsions.map(item => [
    ...
    const ws = XLSX.utils.aoa_to_sheet(wsData);
      ...
      ws['!merges'] = [
       // Fusionne les colonnes 1 à 3 sur la deuxième ligne pour "CUMUL"
        { s: { r: 0, c: 0 }, e: { r: 0, c: 6 } }, // Fusionne les colonnes 1 à 3 sur la première ligne pour "Consolidation des recettes fiscales"
      ];
    
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, `PR_T${selectedTrimestre}_${departementa}_${selectedYear}`);
    XLSX.writeFile(wb, `previsions-${selectedYear}-trimestre-${selectedTrimestre}-${selectedCentre}.xlsx`);
  };
  ...
  return (
    ...
    <div>
      ...
      <Button onClick={exportToExcel} type="default" icon={<FileExcelOutlined />} style={{ marginRight: 16,marginBottom: 20,marginLeft: 500 }}>
        Exporter en Excel
      </Button>
      <Button onClick={exportToPDF}  icon={<FilePdfOutlined />} type="default">
        Exporter en PDF
      </Button>
      <div/>
      <div/>
...
export default PrevisionByTrimestreA;
