...
const PrevisionByTotal = () => {
  ...
  useEffect(() => {
    if (selectedYear && selectedTrimestre && token) {
      axios.get(`http://localhost:5000/prevision/getByTrimestreA/${selectedYear}/${selectedTrimestre}`, {
        headers: {
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, `PR_total_T${selectedTrimestre}_DRI_${selectedYear}`);
    XLSX.writeFile(wb, `previsions total-${selectedYear}-trimestre-${selectedTrimestre}.xlsx`);
  };
  ...
  return (
    ...
    <div>
      <Title level={2} style={{ marginBottom: '16px', textAlign: 'center', color: '#93c4c5' }}>Prévisions totals par Trimestre{}</Title>
      <Select
      ...
      <Button onClick={exportToExcel} type="default" icon={<FileExcelOutlined />} style={{ marginRight: 16,marginBottom: 20 }}>
        Exporter en Excel
      </Button>
...
export default PrevisionByTotal;
