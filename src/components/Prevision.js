...
const PrevisionList = () => {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:5000/prevision/getAll', {
      headers: {
  ...
  }, [searchText, selectedMonth, selectedYear, previsions]);

  const goToTrimestreView = () => {
    navigate('/PrevisionByTrimestre');
  };
  ...
  const handleDelete = (id_prevision) => {
    axios.delete(`http://localhost:5000/prevision/delete/${id_prevision}`, {
      headers: { Authorization: `Bearer ${token}` }
    })
      .then(() => {
    ...
    form.validateFields()
      .then(values => {
        ...
        const url = selectedRecord
          ? `http://localhost:5000/prevision/update/${selectedRecord.id_prevision}`
          : 'http://localhost:5000/prevision/create';
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.autoTable({
      head: [['Types d\'Impôts', 'Montant Prévision', 'Mois', 'Année']],
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, `Prévisions_${nom_departement}`);
    const wbout = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
    saveAs(new Blob([wbout], { type: 'application/octet-stream' }), `previsions_${nom_departement}.xlsx`);
  };
