...
const ConsolidationByTrimestre = () => {
  ...
  const fetchData = () => {
    if (selectedYear && selectedTrimestre && token) {
      axios.post(`http://localhost:5000/recette/getConsolidationTri`, {
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, `CR_T${selectedTrimestre}_${selectedYear} du ${nom_departement} `);
...
export default ConsolidationByTrimestre;
